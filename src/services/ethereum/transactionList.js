...
const WalletTransactionHistory = ({ address }) => {
  ...
  const fetchTransactionHistory = async (address) => {
    setLoading(true);
    try {
      const apiKey = process.env.EXPO_PUBLIC_ETHERSCAN_API_KEY; // Replace with your Etherscan API key
      const response = await axios.get(
        `https://api-holesky.etherscan.io/api?module=account&action=txlist&address=${address}&startblock=0&endblock=99999999&sort=desc&apikey=${apiKey}`
      );
  ...
  const renderTransaction = ({ item }) => (
    ...
    >
      ...
      <Text>
        ...
        {parseFloat(ethers.formatEther(item.value)).toFixed(5)} ETH
      </Text>
      <Text>
